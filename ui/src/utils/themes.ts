...
import { ref } from 'vue';
const { layoutConfig, setPrimary, setSurface, setPreset, setMenuMode, toggleDarkMode, isDarkTheme } = useLayout();
...
export const useThemeStore = defineStore('ThemeStore', () => {
    ...
    function getPresetExt(primaryColorName: string) {
        const color: any = primaryColors.value.find((c) => c.name === primaryColorName);
    ...
    function toggleTheme(darkTheme: boolean, colorName: string) {
        if (darkTheme) {
            toggleDarkMode();
        }
        // const color: any = primaryColors.value.find((c) => c.name === colorName);
